...
function initKanban(){
	...
	$('.kanban-item').draggable({ revert: 'invalid' });
...
function todayPassoDrop(event, ui){
	...
	$(itemDrop).draggable({ revert: "invalid"});
...
function todayDrop(event, ui){
	...
	$.post(baseUrl, {id: id, status: 1}, function(data){
		$(novaAtividade)
			.append(data)
...
function doneDrop(event, ui){
	...
	$.post(baseUrl, {id: id, status: 2}, function(data){
...
function todoDrop(event, ui){
	...
	$.post(baseUrl, {id: id, status: 0}, function(data){
...
function savePasso(){
	var id = $(this).attr('id');	
	$.post('/sipesq/index.php/atividade/passoConcluido/' + id ,{finalizado: this.checked}, function(data){
